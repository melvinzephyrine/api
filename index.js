...
app.use((req, res, next) => {
  ...
  res.on('finish', () => {
    ...
    if (!cleanPath.startsWith('/api/') || cleanPath === '/api/data') {
      return;
    }
    ...
    let statusColored = chalk.black.bgGreen(` ${status} `);
    if (status >= 500) {
      statusColored = chalk.white.bgRed.bold(` ${status} `);
    } else if (status >= 400) {
      statusColored = chalk.black.bgYellow.bold(` ${status} `);
    } else if (status >= 300) {
      statusColored = chalk.black.bgCyan(` ${status} `);
    }

    const method = req.method;
    const methodColored = method === 'POST' 
      ? chalk.black.bgGreen.bold(` ${method} `)
      : chalk.black.bgCyan.bold(` ${method} `);

    const duration = Date.now() - start;
    const featureName = endpointNameMap[`${method}:${cleanPath}`] || cleanPath;

    const fullUrl = `${req.protocol}://${req.get('host')}${req.originalUrl}`;

    console.log(
      `${chalk.gray(`[${timeOnly}]`)} ${methodColored} ${chalk.bold.white(featureName)} ${statusColored} ${chalk.yellow(`${duration}ms`)} ${chalk.cyan(fullUrl)}`
    );
  });
...
const register = (ep, file) => {
  if (ep && ep.name && (ep.desc || ep.description) && ep.category && ep.path && typeof ep.run === "function") {
    ...
    endpointNameMap[`${method.toUpperCase()}:${cleanPath}`] = ep.name;
...
app.use((err, req, res, next) => {
  ...
  if (err.stack) {
    ...
    const projectLine = stackLines.find(line => line.includes(process.cwd()) && !line.includes('node_modules'));
    if (projectLine) {
      const match = projectLine.match(/([^\/\\]+\.js:\d+:\d+)/);
      if (match) {
  ...
  console.error(
    `${chalk.gray(`[${timeOnly}]`)} ${chalk.white.bgRed.bold(' ERROR ')} ${chalk.bold.red(req.method)} ${chalk.bold.white(req.path)} → ${chalk.red(errorMessage)}${chalk.yellow(errorLocation)}`
  );
